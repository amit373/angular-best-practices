...
export class FilterPipe implements PipeTransform {
  static isFoundOnWalking(value: any, key: string) {
    let walker = value;
    let found = false;
    do {
      if (
  ...
  private filterByObject(filter: { [x: string]: any; $or: any[] }) {
    return (value: { [x: string]: any }) => {
  ...
  private filterByOr(filter: any[]): (value: any) => boolean {
    return (value: any) => {
      const length: number = filter.length;
